...
class LineDetectorHSV:
    ...
    def __init__(self):
        ...
        self.hsv_white2 = np.array([250,100,255])
    ...
    def _colorFilter(self, color):
        ...
        # edge_color = self.edges for debugging
    ...
    def _findNormal(self, bw, lines):
        ...
        if len(lines) > 0:
            ...
            length = np.sum((lines[:, 0:2] - lines[:, 2:4]) ** 2, axis=1, keepdims=True) ** 0.5 #pyt theorem
            dx = 1.* (lines[:, 3:4] - lines[:, 1:2]) / length
            dy = 1.* (lines[:, 0:1] - lines[:, 2:3]) / length

            centers = np.hstack([(lines[:, 0:1] + lines[:, 2:3]) / 2, (lines[:, 1:2] + lines[:, 3:4]) / 2]) #get centre coords
